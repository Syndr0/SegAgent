...
function App() {
  ...
  // A mask produced by the SegAgent chat: fetch it and overlay it in the viewer.
  const handleAgentMask = async (maskId: string, prompt: string) => {
    try {
      const response = await fetch(`http://localhost:8000/mask/${maskId}`);
      if (!response.ok) return;
      const blob = await response.blob();
      const file = new File([blob], `agent_${prompt.replace(/\s+/g, '_')}.nii.gz`, { type: 'application/gzip' });

      setSegmentations(prev => {
        const colorObj = SEGMENTATION_COLORS[prev.length % SEGMENTATION_COLORS.length];
        const newSeg: Segmentation = {
          id: `${maskId}`,
          file,
          prompt,
      ...
      console.error('Error fetching agent mask:', error);
    }
  };
